...
(define (cond->if [cs : (Listof (Exp * Exp))]) : ExpPure
  (type-case (Listof (Exp * Exp)) cs
    [empty
     (numEp 42)]
    [(cons c cs)
     (ifEp (remove-sugar (fst c))
          (remove-sugar (snd c))
          (cond->if cs))]))

(define (remove-sugar [e : Exp]) : ExpPure
  (type-case Exp e
    [(numE n) (numEp n)]
    [(opE o l r) (opEp o (remove-sugar l) (remove-sugar r))]
    [(ifE b l r) (ifEp (remove-sugar b) (remove-sugar l) (remove-sugar r))]
    [(condE cs) (cond->if cs)]
    ))
...
(define (eval [e : ExpPure]) : Value
  (type-case ExpPure e
    [(numEp n) (numV n)]
    [(opEp o l r) ((op->proc o) (eval l) (eval r))]
    [(ifEp b l r)
...
(define (main [e : S-Exp]) : Void
  (print-value (eval (remove-sugar (parse e)))))
