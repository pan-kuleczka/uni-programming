...
(define esd : StackDecompile empty)
(define (pushSD [v : Exp] [s : StackDecompile]) : StackDecompile
  (cons v s))
...
(define (decompileVM [c : Code] [s : StackDecompile]) : Exp
  (type-case Code c
    [empty
     (fst (popSD s))]
    [(cons i c)
     (type-case Instr i
       ...
       [(opI op)
        (let* ([n2-s2 (popSD s)]
               ...
               [n1 (fst n1-s1)]
               [s1 (snd n1-s1)]
               [s0 (pushSD (opE op n1 n2) s1)])
          (decompileVM c s0))])]))

(define (decompile [code : Code]) : Exp
    (decompileVM code esd)
)

(module+ test
    (test (parse `1) (decompile (compile (parse `1))))
    (test (parse `(+ 1 1)) (decompile (compile (parse `(+ 1 1)))))
    (test (parse `(+ (* 2 3) 1)) (decompile (compile (parse `(+ (* 2 3) 1)))))
)
