...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{null? ANY} s)
     (nullPE (parse (second (s-exp->list s))))]
    [(s-exp-match? `{list ANY ...} s)
     (listE (map parse (rest (s-exp->list s))))]
  ...
  (test (parse `{cond {{= 0 1} {* 3 4}}
       ...
       (condE (list (pair (opE (eql) (numE 0) (numE 1))
                   ...
                   (pair (opE (eql) (numE 1) (numE 1))
                        (numE 8)))))
  (test (parse `{list {cons null 2} {car {cons 2 3}}})
       (listE (list
               (consE (nullE) (numE 2))
               (carE (consE (numE 2) (numE 3)))
               ))
       ))
...
(define (eval [e : Exp]) : Value
  (type-case Exp e
    ...
    [(nullE) (listV empty)]
    [(nullPE e) (boolV
                 (let* [(x (eval e))] (and (listV? x) (empty? (listV-s x))))
                 )]
    [(listE s) (listV (map eval s))]
    [(consE l r) (consV (eval l) (eval r))]
    [(carE e) (let* [(x (eval e))] (type-case Value x
                                     [(numV n) (error 'eval "car violation: not a list")]
                                     [(boolV b) (error 'eval "car violation: not a list")]
                                     [(consV l r) l]
                                     [(listV s) (error 'eval "car violation: not a list")]
                                     ))]
    [(cdrE e) (let* [(x (eval e))] (type-case Value x
                                     [(numV n) (error 'eval "cdr violation: not a list")]
                                     [(boolV b) (error 'eval "cdr violation: not a list")]
                                     [(consV l r) r]
                                     [(listV s) (error 'eval "cdr violation: not a list")]
                                     ))]
...
(define (value->string [v : Value]) : String
  (type-case Value v
    ...
    [(consV l r) (string-append (string-append
    (string-append (string-append "'(" (value->string l)) " . ")
    (value->string r))")")]
    [(listV s) (string-append "'("
                             (string-append
                              (foldr
                               (λ (a b) (string-append " "
                                         (string-append a b))
